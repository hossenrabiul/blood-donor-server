...
class UserView(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        user_id = self.request.query_params.get('id')  # Get the 'id' query parameter

        if user_id:
            queryset = queryset.filter(id=user_id)  # Filter the queryset by 'id'
...
class UserRegistrationApiView(APIView):
    ...
    def post(self,requert):
        ...
        if serializer.is_valid():
            ...
            print("UID: ",uid)
            confirm_link = f"https://datadonor-webapp.vercel.app/accounts/active/{uid}/{token}"
            ...
            email = EmailMultiAlternatives(
                ...
                body='',
                from_email= 'Data.Doner <noreply@example.com>', 
                to =[user.email]
            )
