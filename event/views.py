...
class EventView(viewsets.ModelViewSet):
    ...
    def event_create(self, request):
        ...
        try:
            ...
            body = render_to_string('mail_event_created.html',context) # Adds alternative HTML content 

            for profile in profile_users:
                try: 
                    email = EmailMultiAlternatives(
                        subject=f"Blood Event of {event.blood}",
                        body='',
    ...
    def blood_received(self,request,pk=None):
        ...
        context = {
            'donor_name': event.user.username,
            'host_name': event.doner.username,
            'time': current_date,
            'location': event.location, 
        }
